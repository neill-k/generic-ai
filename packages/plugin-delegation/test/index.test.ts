...
import { SessionOrchestrator } from "@generic-ai/core";

import { createDelegationCoordinator, kind, name } from "../src/index.js";
...
describe("@generic-ai/plugin-delegation", () => {
  it("delegates work through child sessions while the kernel orchestrator owns lifecycle", async () => {
    ...
    expect(coordinator.name).toBe(name);
    expect(coordinator.kind).toBe(kind);
