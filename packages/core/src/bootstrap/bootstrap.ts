...
function createPrimaryAgentPlan(
  ...
  if (agentId !== undefined) {
    return normalizeAgentPlan(agentId, config.agents[agentId]);
  }
...
function createRuntimePlan<TSchemaSource, TRuntimeStart>(
  ...
  return Object.freeze({
    runtime: createRuntimeSettingsPlan(config, options.configSource?.startDir),
    primaryAgent: createPrimaryAgentPlan(config, options.primaryAgentId),
    plugins: createPluginInitPlans(config),
