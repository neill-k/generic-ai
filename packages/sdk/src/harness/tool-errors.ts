    ...
    const status =
      typeof source.status === "number"
        ...
        : typeof source.statusCode === "number"
          ? source.statusCode
          : undefined;
    const metadata = jsonObjectFor(source.metadata);
...
function jsonObjectFor(value: unknown): JsonObject | undefined {
...
export function normalizeToolError(input: NormalizeToolErrorInput): ToolErrorEnvelope {
  if (input.error instanceof GenericAIToolError) {
    const kind = input.kind ?? input.error.envelope.kind;
    const preserveClassification = kind === input.error.envelope.kind;
    const metadata = mergeMetadata(input.error.envelope.metadata, input.metadata);

    return Object.freeze({
      kind,
      safeMessage: input.safeMessage ?? input.error.envelope.safeMessage,
      retryable: preserveClassification
        ? input.error.envelope.retryable
        : defaultRetryable(kind),
      transient: preserveClassification
        ? input.error.envelope.transient
        : defaultTransient(kind),
      userActionable: preserveClassification
        ? input.error.envelope.userActionable
        : defaultUserActionable(kind),
      ...(input.error.envelope.rawCause === undefined
        ? {}
        : { rawCause: input.error.envelope.rawCause }),
      ...(input.remediationHints === undefined
        ? input.error.envelope.remediationHints === undefined
          ? {}
          : { remediationHints: input.error.envelope.remediationHints }
        : { remediationHints: Object.freeze([...input.remediationHints]) }),
      ...(input.timeoutBudget === undefined
        ? input.error.envelope.timeoutBudget === undefined
          ? {}
          : { timeoutBudget: input.error.envelope.timeoutBudget }
