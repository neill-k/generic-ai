...
The default path remains the starter Hono composition. To keep `createGenericAI()` usable with no preset input, the core bootstrap layer carries a mirrored internal starter descriptor as an approved bootstrap-only exception. That exception is limited to preset metadata and plugin IDs: `@generic-ai/core` still must not import preset or plugin packages, and `@generic-ai/preset-starter-hono` remains the package-owned public starter contract for callers who want to make the default path explicit.
...
- Boundary enforcement must keep plugin and preset package imports out of the kernel even though the mirrored starter descriptor lives in bootstrap metadata.
